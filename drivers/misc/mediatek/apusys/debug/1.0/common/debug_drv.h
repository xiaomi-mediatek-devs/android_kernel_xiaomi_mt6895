...
pr_debug(APUSYS_DEBUG_LOG_PREFIX "[error] %s " x, __func__, ##args)
#define LOG_WARN(x, args...) \
pr_debug(APUSYS_DEBUG_LOG_PREFIX "[warn] %s " x, __func__, ##args)
#define LOG_INFO(x, args...) \
pr_debug(APUSYS_DEBUG_LOG_PREFIX "[info] %s " x, __func__, ##args)
#define DEBUG_TAG LOG_DEBUG("\n")

#define LOG_DEBUG(x, args...) \
	{ \
		if (debug_log_level > 0) \
			pr_debug(APUSYS_DEBUG_LOG_PREFIX "[debug] %s/%d "\
...
#define LOG_DETAIL(x, args...) \
	{ \
		if (debug_log_level > 1) \
			pr_debug(APUSYS_DEBUG_LOG_PREFIX "[detail] %s/%d "\
			x, __func__, __LINE__, ##args); \
	}
...
#define INT_STA_PRINTF(m, x, args...)\
	{ \
		if (m != NULL) \
			...
			pr_debug(APUSYS_DEBUG_LOG_PREFIX "[isr_work] %s/%d "\
			x, __func__, __LINE__, ##args); \
	}
