...
#define KTD2687_NAME	"ktd2687_right"
...
/* led1/2 enable/disable */
static int ktd2687_right_single_enable_ctrl(struct ktd2687_flash *flash,
			      enum ktd2687_led_id led_no, bool on)
{
	int rval;
...
/* torch1/2 brightness control */
static int ktd2687_right_single_torch_brt_ctrl(struct ktd2687_flash *flash,
				 enum ktd2687_led_id led_no, unsigned int brt)
{
	int rval;
	u8 br_bits;
			      ...
			      enum ktd2687_led_id led_no, char *led_name)
{
	...
	const char *fled_name = "frontrightflash";
	int rval;
...
static int ktd2687_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_SET_ONOFF:
		...
		break;
	case XIAOMI_IOC_SET_FRONT_ONOFF:
		pr_debug("XIAOMI_IOC_SET_FRONT_ONOFF(%d): %d\n",
				channel, (int)fl_arg->arg);
		if ((int)fl_arg->arg) {
			ktd2687_enable_ctrl(ktd2687_flash_data, channel, true);
		} else {
	...
	case XIAOMI_IOC_SET_SINGLE_FRONT_ONOFF:
		pr_debug("XIAOMI_IOC_SET_SINGLE_FRONT_ONOFF(%d): %d\n",
				channel, (int)fl_arg->arg);
		if ((int)fl_arg->arg) {
			ktd2687_right_single_enable_ctrl(ktd2687_flash_data, channel, true);
		} else {
			ktd2687_right_single_enable_ctrl(ktd2687_flash_data, channel, false);
		}
		break;
	case XIAOMI_IOC_SET_TORCH_FRONT_CUR:
		pr_debug("XIAOMI_IOC_SET_TORCH_FRONT_CUR(%d): %d\n",
				channel, (int)fl_arg->arg);
		ktd2687_torch_brt_ctrl(ktd2687_flash_data, channel, fl_arg->arg);
		break;
	case XIAOMI_IOC_SET_SINGLE_TORCH_FRONT_CUR:
		pr_debug("XIAOMI_IOC_SET_SINGLE_TORCH_FRONT_CUR(%d): %d\n",
				channel, (int)fl_arg->arg);
		ktd2687_right_single_torch_brt_ctrl(ktd2687_flash_data, channel, fl_arg->arg);
		break;
	case XIAOMI_IOC_SET_FRONT_MODE:
		pr_debug("XIAOMI_IOC_SET_FRONT_MODE(%d): %d\n",
				channel, (int)fl_arg->arg);
			...
			const struct i2c_device_id *devid)
{
	...
	rval = ktd2687_subdev_init(flash, KTD2687_LED0, "ktd2687right-led0");
	if (rval < 0)
		return rval;

	rval = ktd2687_subdev_init(flash, KTD2687_LED1, "ktd2687right-led1");
	if (rval < 0)
		return rval;
...
static const struct of_device_id ktd2687_of_table[] = {
	{ .compatible = "mediatek,ktd2687_right" },
	{ },
};
